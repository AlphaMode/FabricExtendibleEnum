...
public class RuntimeEnumExtender {
    ...
    private final Type MARKER_IFACE = Type.getType("Lnet/alphamode/enums/IExtensibleEnum;");
    ...
    private final Type UNSAFE_HACKS = Type.getType("Lnet/alphamode/enums/UnsafeHacks;"); //Again, not direct reference to prevent class loading.
    ...
    public static Enum<?> createEnum(Class<?> enumClass, Class... args) {
        Enum<?> enom = null;
        try {
            enom = (Enum<?>) enumClass.getDeclaredMethod("create", args).invoke(null, args);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return enom;
...
//                }));
            ...
            {
                LOGGER.fatal(
                    "Enum has create method with no matching constructor:\n" +
