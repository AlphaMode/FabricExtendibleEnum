...
public interface IExtensibleEnum<T>
{
    ...
    /**
     * Use this instead of {@link StringIdentifiable#createCodec(Supplier, Function)} for extensible enums because this not cache the enum values on construction
     */
    static <E extends Enum<E> & StringIdentifiable> Codec<E> createCodecForExtensibleEnum(Supplier<E[]> valuesSupplier, Function<? super String, ? extends E> enumValueFromNameFunction) {
        return Codec.either(Codec.STRING, Codec.INT).comapFlatMap(
                ...
                value -> Either.left(value.asString())
        );
    }
